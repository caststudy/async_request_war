...
var app = {
   ...
   init: function() {	
      $('login-name').focus();
//      app.listen();  20190626 comment
   },
   listen: function(name) {
      $('comet-frame').src = app.url + '?name='+name+"&count="+ + count;
      count ++;
   },
   login: function() {
...
//		    alert(typeof msg.responseText)
		    var res= eval("("+ msg.responseText+")");
//		    alert(typeof res)
		    if(res.status=="success") app.listen(res.name);	//20190626 add 	 
   ...
   post: function() {
	 ...
	 onComplete: function(msg) {		
	    ...
	    $('message').value = '';
	    var res= eval("("+ msg.responseText+")");
		if(res.status=="failure") app.listen(res.name);
	 }
//      ,
...
//	 },
//	 onSuccess: function(msg) {
//		 alert('s'+msg.responseText+msg.status+msg.readyState);
//	 }
...
Behaviour.addLoadEvent(app.init);//聚焦'login-name'编辑框
